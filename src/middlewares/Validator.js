...
module.exports = {
  ...
  async AreaValidator(req, res, next) {
    try {
      const schema = Yup.object().shape({
        nomeArea: Yup.string().required(),
        descricaoArea: Yup.string().required(),
      });
      await schema.validate(req.body, { abortEarly: false });
      return next();

    } catch (error) {
      return res.status(400).json({ message: 'Invalid data types to Area' });
    }
  },
  async EventosValidator(req, res, next) {
    try {
      const schema = Yup.object().shape({
      ...
      return res.status(400).json({ message: 'Invalid data types to Eventos' });
    }
